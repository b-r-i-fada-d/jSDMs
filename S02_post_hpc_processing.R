...
parameter_file_path <- paste0("_parameter_estimates.txt")
parameter_file_path <- file.path(parameter_file_path) 
cat("This file contains additional information regarding parameter estimates.\n\n",
    file = parameter_file_path, sep = "")

parameter_plots_path <- paste0("_paramater_estimates.pdf")
parameter_plots_path <- file.path(parameter_plots_path) 
...
if(model$nc > 1) {
  ...
  beta_filename <- paste0("_parameter_estimates_Beta.xlsx")
  beta_filename <- file.path(beta_filename)
...
if(model$nr > 0 && model$ns > 1) {
  ...
  for (r in seq_len(model$nr)) {
    ...
    omega_filename <- paste0("_parameter_estimates_Omega_", names(model$ranLevels)[r],".xlsx")
    omega_filename <- file.path(omega_filename)
