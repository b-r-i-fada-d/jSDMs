...
env <- read_csv("stations_monthly_complete.csv")
...
df <- df %>%
  mutate(dummy_sub = as.numeric(factor(Substrate)))
...
m_SPACE = Hmsc(Y = Y, XData = XData, XFormula = ~1, # this is the difference
               distr = "probit",
               studyDesign = studyDesign,
               ranLevels = list(year = rL.year))

# Environmental covariates with no spatial random effect
m_ENV = Hmsc(Y = Y, XData = XData, XFormula = XFormula,
             distr = "probit")
